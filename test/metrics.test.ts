...
test("TPS 从 Turn 开始到完成计算，包含 TTFT 与工具等待", () => {
  const result = calculateMetrics({ durationMs: 10_000, ttftMs: 2_000, outputTokens: 20 });
  assert.equal(result.tps, 2);
  assert.equal(result.durationMs, 10_000);
});

test("缺少 TTFT 仍可计算 TPS，缺少总时长或输出时为 N/A", () => {
  assert.equal(calculateMetrics({ durationMs: 1_000, ttftMs: null, outputTokens: 10 }).tps, 10);
  assert.equal(calculateMetrics({ durationMs: 0, ttftMs: 0, outputTokens: 10 }).tps, null);
