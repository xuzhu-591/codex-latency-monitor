...
菜单栏始终展示最近完成的一轮的来源和模型，例如：

```text
cx · gpt-5.6-sol · 7.8s · 21.2/s
```

展开后，最近 10 轮会标注 `cx`（Codex）或 `cc`（Claude Code）、模型、TTFT 和 TPS 的数值，但不重复标出指标名；当天汇总按“来源 + 模型”分别展示带指标名的 p50/p95 和 p50/p5。

本地 HTML 报告展示昨天零点至当前的时序图和最近 50 轮。图中的 `● cx` 与 `◆ cc` 标记、表格来源/模型列和悬停提示均可区分数据来源与模型；报告表格展示真实会话 ID，菜单栏不展示。
