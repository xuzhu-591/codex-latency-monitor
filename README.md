...
在 macOS 菜单栏被动查看本机 **Codex** 和 **Claude Code** 的响应体验：TTFT 与 Effective TPS。
...
- **Effective TPS**：该 Turn 的输出 token 总数 ÷ 用户提交到 Turn 完成的总时长，越大越好。它包含 TTFT、模型思考、网络与工具等待，代表从“提问”到“拿到结果”的端到端效率。
...
```text
Claude · TTFT 2.8s · Effective TPS 12.5/s
```

展开后，最近 10 轮会明确标注 `Codex` 或 `Claude`。当天汇总展示 TTFT p50/p95 与 Effective TPS p50/p5。
